...
const AirdropPrizeList = () => {
  ...
  const [danceStatus, setDanceStatus] = useState<boolean>(false);
  ...
  const handleDanceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked;
    setDanceStatus(checked);
    setFormData((prevData) => ({
      ...prevData,
      dance: checked,
    }));
    ...
    await apiClient
      ...
      .then((res) => {
        ...
        if (res.data.success) {
          ...
          setFakeTotalProbability(data.map((e: prizelist) => e.fake_probability).reduce((accumulator: number, currentValue: number) => accumulator + currentValue, 0))
      ...
      .then((res) => {
        ...
        if (data) {
          setFormData((prevData) => ({
            ...
            ca: data.ca,
            icon: data.icon,
            dance: data.dance || false,
          }));
          setDanceStatus(data.dance || false);
        }
      })
      .catch((err) => {
        console.log(err);
    ...
    (prize: prizelist, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case "fake_probability":
          return <p className="text-center"> {prize.fake_probability}</p>;
        case "quantity":
  ...
  const onEditPrize = async (formData: prizeEditItem) => {
    ...
    data.append("dance", String(formData.dance));
