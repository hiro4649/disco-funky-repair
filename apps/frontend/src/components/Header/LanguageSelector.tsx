  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      <li>
        ...
        {dropdownOpen && (
          <div
            className={`absolute right-0 mt-3 flex w-[364px] flex-col rounded-xl border-[0.5px] border-stroke bg-white px-2 sm:px-4 py-4 shadow-default dark:border-white dark:bg-mainbg sm:right-0 language-selector-dropdown z-1`}
          >
                ...
                <li key={index}>
                  ...
                  >
                    <span className="block">
                      <span className={`block text-[16px] ${item.label == languageLocale ? "text-main" : item.available ? "text-[#ffffff]" : "text-[#666666]"}`}>
                        {item.title}
                      </span>
                    </span>
                  </div>
