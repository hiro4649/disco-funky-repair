...
export default function DefaultLayout({
  ...
  return (
    <div className={`${openScreen == true ? "loaded" : ""}`}>
      ...
      <>
        {/* <!-- ===== Page Wrapper Star ===== --> */}
        <div ref={mainContentRef} className={`${drawState ? "overflow-hidden" : "flex h-[100dvh]"}`}>
          {/* <!-- ===== Sidebar Star ===== --> */}
          <Sidebar
          ...
          <div className="relative w-full bg-mainbg">
            ...
            <main
              ref={mainContentRef}
              className={`bg-mainbg ${drawLoading ? "overflow-hidden" : ""}`}
            >
              <div className="mt-[88px]">{children}</div>
            </main>
            {/* <!-- ===== Main Content End ===== --> */}
            <footer className="flex items-center justify-center py-9 dark:bg-mainbg bg-white">
              <div>
                ...
                <p className="text-center text-xs text-white">
                  © 2025 {process.env.NEXT_PUBLIC_APP_NAME} RAVE All rights reserved.
                </p>
              </div>
            </footer>
          </div>
