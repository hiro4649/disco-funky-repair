...
import {
    processSixHourTokenBalance,
    processWeeklyBonus
} from '../lib/trackingTokenBalanceEthereum';
import {
    setProbability,
    registerAllEthereumTokens
} from '../lib/trackingTokensEthereum';
...
import {
    ...
    stopRealtimeEventListener
} from '../lib/realtimeEventListener';
import {
    processScheduledTierUpdates,
    cleanupOldScheduledUpdates
} from '../lib/tierScheduler';
...
type TrackingScheduledTask = cron.ScheduledTask & {
    destroy?: () => void;
};

let trackingSchedulersStarted = false;
let trackingSchedulerTasks: TrackingScheduledTask[] = [];

const scheduleTrackingTask = (
    expression: string,
    task: Parameters<typeof cron.schedule>[1]
): TrackingScheduledTask => {
    const scheduledTask = cron.schedule(expression, task, {
        timezone: 'UTC'
    }) as TrackingScheduledTask;
    trackingSchedulerTasks.push(scheduledTask);
    return scheduledTask;
};

export const startTrackingSchedulers = (): void => {
    if (trackingSchedulersStarted) {
        return;
    }

    trackingSchedulersStarted = true;

    // Six-hour token balance update - runs every 6 hours
    scheduleTrackingTask('0 */6 * * *', async () => {
    ...
    scheduleTrackingTask('0 22 * * *', async () => {
    ...
    scheduleTrackingTask('0 2 * * *', async () => {
        const currentTime = moment.utc().format('YYYY-MM-DD HH:mm:ss');
    ...
    scheduleTrackingTask('0 3 * * *', async () => {
        const currentTime = moment.utc().format('YYYY-MM-DD HH:mm:ss');
...
export const stopTrackingSchedulers = async (): Promise<void> => {
    for (const task of trackingSchedulerTasks) {
        task.stop();
        task.destroy?.();
    }

    trackingSchedulerTasks = [];
    trackingSchedulersStarted = false;

    try {
        await stopRealtimeEventListener();
    } catch (error) {
        safeLogError('tracking_stop_realtime_event_listener', error);
    }
};
