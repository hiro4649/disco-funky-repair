...
export function buildWorkflowLegacySelfTestOptionalReport(input = parseJson(process.env.CODEX_WORKFLOW_LEGACY_SELF_TEST_OPTIONAL_JSON) || {}) { const reasonCodes = []; const text = readText('scripts/codex-workflow-quality-runner.mjs') || ''; if (!/v099OptionalNotApplicable[\s\S]*v099SelfTestStatus/.test(text) || !/optionalNotApplicable[\s\S]*v085SelfTestStatus/.test(text)) reasonCodes.push('workflow_legacy_self_test_optional_failed'); if (parseBool(input.legacyNotApplicableStillBlocks)) reasonCodes.push('workflow_legacy_self_test_optional_failed'); return safe('workflowLegacySelfTestOptionalStatus', reasonCodes.length ? 'fail' : 'pass', { reasonCodes }); }
