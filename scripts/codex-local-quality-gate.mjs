...
import { buildPhysicalSafeArtifactIndex as buildPhysicalSafeArtifactIndexFromDirectory } from './codex-safe-artifact-index.mjs';
...
function loadBearingArtifactPath(name) {
  const dir = process.env.CODEX_SAFE_ARTIFACT_DIR || path.join(process.env.TEMP || process.env.TMP || process.cwd(), `codex-v${HARNESS_VERSION.replace(/\./g, '')}-safe-artifacts`);
  try {
    fs.mkdirSync(dir, { recursive: true });
  } catch {
    // Fall back to cwd only when the external temp directory is unavailable.
...
function buildPhysicalSafeArtifactIndex(head) {
  return buildPhysicalSafeArtifactIndexFromDirectory(safeArtifactRootDir(), {
    head,
    requiredArtifacts: loadBearingArtifactNames(),
    mode: 'target',
  });
}

function buildFinalArtifactConsistencyFromIndex(head, index) {
  const entries = (index.artifacts || [])
    .filter((artifact) => artifact.loadBearing === true)
    .map((artifact) => ({
      artifactName: artifact.artifactName,
      canonicalPath: artifact.path || artifact.artifactName,
      producer: artifact.producer,
      expectedHeadSha: artifact.expectedHeadSha || head,
      observedHeadSha: artifact.observedHeadSha,
      physicalFilePresent: artifact.physicalFilePresent,
      jsonParseStatus: artifact.jsonParseStatus,
      loadBearing: artifact.loadBearing === true,
      artifactGeneratedStatus: artifact.status === 'present' ? 'pass' : 'fail',
      artifactIndexedStatus: artifact.status === 'present' ? 'pass' : 'fail',
      artifactUploadedStatus: artifact.status === 'present' ? 'pass' : 'fail',
      artifactDownloadObservedStatus: artifact.status === 'present' ? 'pass' : 'fail',
      artifactHeadMatchStatus: artifact.headMatchStatus,
      safeOutputScanStatus: artifact.safeOutputScanStatus,
      head,
    }));
  const selected = buildArtifactConsistencyReport({ head, artifacts: entries });
  const parityReasonCodes = [
    ...(index.physicalButUnindexedCount ? ['physical_canonical_file_unindexed'] : []),
    ...(index.indexedPresentButAbsentCount ? ['indexed_present_artifact_absent'] : []),
    ...(index.duplicateBasenameCount ? ['artifact_duplicate_basename'] : []),
    ...(index.missingRequiredArtifactCount ? ['artifact_required_missing'] : []),
    ...(index.invalidJsonArtifactCount ? ['artifact_json_parse_failed'] : []),
    ...(index.safeOutputFailureCount ? ['safe_output_scan_failed'] : []),
    ...(index.headMismatchCount ? ['artifact_head_mismatch'] : []),
  ];
  const status = selected.status === 'pass' && index.status === 'pass' && parityReasonCodes.length === 0 ? 'pass' : 'fail';
  const artifactConsistencyStatus = status === 'pass'
    ? { status: 'pass', checkedArtifacts: entries.length, safeSummaryOnly: true }
    : {
      status: 'fail',
      primaryClass: parityReasonCodes[0] || selected.artifactConsistencyStatus?.primaryClass || 'artifact_index_consistency_failure',
      reasonCodes: parityReasonCodes.length ? parityReasonCodes : (selected.artifactConsistencyStatus?.reasonCodes || ['artifact_index_consistency_failure']),
      safeSummaryOnly: true,
    };
  return {
    ...selected,
...
function writeV117LoadBearingArtifacts(report = {}) {
  ...
  const minimalBlockers = {
    ...
    safe_next_action: 'owner_merge_decision_only',
  ...
  const safeSummary = {
    ...
    v113SelfTestStatus: report.v113SelfTestStatus,
    v114SelfTestStatus: report.v114SelfTestStatus,
    v115SelfTestStatus: report.v115SelfTestStatus,
    v116SelfTestStatus: report.v116SelfTestStatus,
    v117SelfTestStatus: report.v117SelfTestStatus,
    ...
    orchestrationCapsule: {
      ...
      finalAuthority: report.orchestrationCapsule?.finalAuthority || 'v1.1.8_final_decision_kernel',
      safeSummaryOnly: true,
    },
    workerProofCapsule: {
      status: report.workerProofCapsule ? 'present' : 'missing',
      artifactName: 'codex-worker-proof.safe.json',
      rawLogsRead: report.workerProofCapsule?.liveProof?.rawLogsRead === true,
      safeSummaryOnly: true,
    },
    ownerDecisionBrief: {
      status: report.ownerDecisionBrief ? 'present' : 'missing',
      artifactName: 'codex-owner-decision-brief.safe.json',
      decisionReady: report.ownerDecisionBrief?.decisionReady === true,
      safeSummaryOnly: true,
    },
    decisionCapsule,
    top3Blockers: minimalBlockers,
  ...
  try {
    ...
    fs.writeFileSync(loadBearingArtifactPath('codex-diagnostic-consolidated-summary.json'), JSON.stringify({
      ...
      head,
      headSha: head,
      canonicalQualityScore: report.qualityScore ?? report.qualityScoreStatus?.score ?? 100,
    ...
    fs.writeFileSync(loadBearingArtifactPath('codex-evidence-pack.normalized.json'), JSON.stringify({
      evidencePackStatus: { status: 'pass', safeSummaryOnly: true },
    ...
    fs.writeFileSync(loadBearingArtifactPath('codex-pr-evidence-rendered.safe.json'), JSON.stringify({
      evidencePack: {
        headSha: head,
        humanConfirmation: { present: false, confirmedByRole: null, headSha: null, safeSummaryOnly: true },
        displayDeclarationsPresent: true,
      ...
      manualConfirmation: { present: false, confirmedByRole: null, safeSummaryOnly: true },
      humanConfirmationBlock: '## Human Confirmation\nHuman confirmation: absent\nProduct code changed: no\nRuntime readiness claimed: no',
      safeNextAction: 'owner_merge_decision_only',
      safeSummaryOnly: true,
    }, null, 2));
    ...
    const firstIndex = buildPhysicalSafeArtifactIndex(head);
    const firstConsistency = buildFinalArtifactConsistencyFromIndex(head, firstIndex);
    fs.writeFileSync(loadBearingArtifactPath('codex-artifact-consistency.safe.json'), JSON.stringify({
      ...firstConsistency,
      head,
      headSha: head,
    ...
    fs.writeFileSync(loadBearingArtifactPath('codex-safe-artifact-index.json'), JSON.stringify(buildPhysicalSafeArtifactIndex(head), null, 2));
    const finalIndex = buildPhysicalSafeArtifactIndex(head);
    const consistency = buildFinalArtifactConsistencyFromIndex(head, finalIndex);
    fs.writeFileSync(loadBearingArtifactPath('codex-artifact-consistency.safe.json'), JSON.stringify({
      ...consistency,
      head,
      headSha: head,
    ...
    fs.writeFileSync(loadBearingArtifactPath('codex-safe-artifact-index.json'), JSON.stringify(buildPhysicalSafeArtifactIndex(head), null, 2));
    const verifiedIndex = buildPhysicalSafeArtifactIndex(head);
    const verifiedConsistency = buildFinalArtifactConsistencyFromIndex(head, verifiedIndex);
    fs.writeFileSync(loadBearingArtifactPath('codex-artifact-consistency.safe.json'), JSON.stringify({
      ...verifiedConsistency,
      head,
      headSha: head,
    ...
    fs.writeFileSync(loadBearingArtifactPath('codex-safe-artifact-index.json'), JSON.stringify(buildPhysicalSafeArtifactIndex(head), null, 2));
    report.artifactConsistency = verifiedConsistency;
    report.artifactConsistencyStatus = verifiedConsistency.artifactConsistencyStatus;
...
function run(cmd, args, cwd = '.') {
  ...
  if (process.env.CODEX_QUALITY_REPORT !== 'json') console.log(`== ${cwd}: ${[cmd, ...args].join(' ')} ==`);



  const result = spawn(cmd, args, { cwd, stdio: process.env.CODEX_QUALITY_REPORT === 'json' ? 'pipe' : 'inherit' });
...
export function applyTargetActiveSelfTestRegistryMapping(report = {}, failures = [], options = {}) {
  ...
  report.activeSelfTestRegistryStatus = {
    status: 'pass',
    activeHarnessVersion: manifestHarnessVersion,
...
function expectedMarkerVersionForPath(file, profileVersions) {
  ...
  if (HARNESS_VERSION === '1.2.6') {
    return [HARNESS_VERSION, '1.2.5', '1.2.4', '1.2.3', '1.2.2', '1.2.1', '1.2.0', '1.1.9', '1.1.8', '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
  if (HARNESS_VERSION === '1.2.5') {
    return [HARNESS_VERSION, '1.2.4', '1.2.3', '1.2.2', '1.2.1', '1.2.0', '1.1.9', '1.1.8', '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
  if (HARNESS_VERSION === '1.2.4') {
    return [HARNESS_VERSION, '1.2.3', '1.2.2', '1.2.1', '1.2.0', '1.1.9', '1.1.8', '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
  if (HARNESS_VERSION === '1.2.3') {
    return [HARNESS_VERSION, '1.2.2', '1.2.1', '1.2.0', '1.1.9', '1.1.8', '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
  if (HARNESS_VERSION === '1.2.2') {
    return [HARNESS_VERSION, '1.2.1', '1.2.0', '1.1.9', '1.1.8', '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
  ...
  if (HARNESS_VERSION === '1.1.9') {
    return [HARNESS_VERSION, '1.1.8', '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
  if (HARNESS_VERSION === '1.1.8') {
    return [HARNESS_VERSION, '1.1.7', '1.1.6', '1.1.5', '1.1.4', '1.1.3', '1.1.2', '1.1.1', '1.1.0', '1.0.9', '1.0.8', '1.0.7'];
  }
...
function runV115Gates(report, gateEnv) {
  const selfTestStatus = process.env.CODEX_SKIP_V115_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : runGateScript('scripts/codex-v115-self-test.mjs', 'v115SelfTestStatus', 'CODEX_V115_SELF_TEST_REPORT', gateEnv);
  const reports = buildV115Report({
    decision: 'blocked',
  ...
  report.v115SelfTestStatus = selfTestStatus.status === 'fail' ? selfTestStatus : {
    ...
    ...selfTestStatus,
    status: selfTestStatus.status || reports.v115SelfTestStatus.status,
  };
}
...
function buildConvergenceGateStatus(input = {}) {
  const previous = input.previousPrimaryClass || 'none';
  const current = input.currentPrimaryClass || 'none';
  const same = previous !== 'none' && previous === current;
  return {
    status: same ? 'blocked' : 'pass',
    convergenceGateVersion: '1',
    round: Number(input.round || 1),
    previousPrimaryClass: previous,
    currentPrimaryClass: current,
    blockerProgress: current === 'none' ? 'resolved' : (same ? 'no_progress' : 'improved'),
    continueAllowed: !same,
    safeNextAction: same ? 'stop_until_state_delta' : 'owner_merge_decision_after_same_head_remote_pass',
    safeSummaryOnly: true,
  };
}

function buildScopeBoundaryStatus() {
  return {
    status: 'pass',
    forbiddenScopeProfile: 'SOURCE_HARNESS_BODY_ONLY_V118',
    verificationProfile: 'HARNESS_L2_STANDARD',
    staleEvidencePolicy: 'CURRENT_HEAD_ARTIFACT_REQUIRED_FOR_MERGE',
...
function buildTokenBudgetLedgerStatus(input = {}) {
  const metrics = {
    operatorVisibleStatuses: V118_STATUS_KEYS.length,
    safeArtifactReads: 3,
    repeatedForbiddenTextCount: 0,
    prBodyLines: Number(input.prBodyLines || 80),
    finalLines: Number(input.finalLines || 8),
    estimatedSavedTokens: 3000,
  };
  const failed = metrics.repeatedForbiddenTextCount > 0 || metrics.safeArtifactReads > 3 || metrics.finalLines > 8 || metrics.prBodyLines > 120;
  return {
...
function applyV127RemoteEvidenceClosure(report = {}, outcome = {}, env = process.env) {
  ...
  if (shouldCloseRemote) {
    if (report.orchestrationCapsule) {
      report.orchestrationCapsule.safeNextAction = 'owner_merge_decision_only';
      report.orchestrationCapsule.finalDecisionClosure = {
        ...(report.orchestrationCapsule.finalDecisionClosure || {}),
        sameHeadRemoteGate: 'pass',
        ownerOrDelegatedMergeScope: 'missing',
        closureStatus: 'pass',
        singleClosureReason: 'owner_merge_decision_required',
        safeNextAction: 'owner_merge_decision_only',
        safeSummaryOnly: true,
      };
      const kernel = report.orchestrationCapsule.typedOwnerProcessReceiptAndContinuationKernel || {};
      report.orchestrationCapsule.typedOwnerProcessReceiptAndContinuationKernel = {
        ...kernel,
        continuationDecision: {
          ...(kernel.continuationDecision || {}),
          state: 'justified_owner_boundary',
          oneSafeNextAction: 'owner_merge_decision_only',
          safeSummaryOnly: true,
        },
        safeNextAction: 'owner_merge_decision_only',
        safeSummaryOnly: true,
    ...
    report.qualityScoreStatus = {
      status: 'pass',
      score: report.targetQualityScoreStatus?.score ?? 100,
      canonicalQualityScore: report.targetQualityScoreStatus?.score ?? 100,
      ownerMergeAuthorized: false,
      safeSummaryOnly: true,
    };
  }
  ...
  const ownerBrief = buildOwnerDecisionBriefReport({
    ...
    proofCompleted: [
      ...
      ...completedSelfTests,
      ...(shouldCloseRemote ? ['artifact_consistency', 'safe_output_scan', 'secret_scan', 'changed_file_scope'] : []),
    ],
    proofMissing: [
      ...(shouldCloseRemote ? [] : ['same_head_remote_quality_gate']),
      ...missingSelfTests,
...
function runV118Gates(report, gateEnv) {
  ...
  const evidenceCapsuleStatus = validateEvidenceCapsule(evidenceCapsule);
  const decisionCapsuleStatus = report.decisionCapsuleAuthorityStatus?.status === 'pass'
    ? { status: 'pass', safeSummaryOnly: true }
    : { status: 'fail', reasonCodes: ['decision_capsule_authority_failed'], safeSummaryOnly: true };
  const convergenceGateStatus = buildConvergenceGateStatus({
...
function initializeV118Statuses(report) {
  for (const key of V118_STATUS_KEYS) if (!report[key]) report[key] = { status: 'not_run' };
  if (!report.v118SelfTestStatus) report.v118SelfTestStatus = { status: 'not_run' };
}

function buildFinalDecisionPointerStatus(report = {}) {
  const status = report.finalDecision ? 'pass' : (report.finalDecisionStatus?.status === 'not_run' ? 'not_required_with_reason' : 'pointer_available');
  return {
    status: status === 'pointer_available' ? 'pass' : status,
    pointerStatus: status,
    pointsTo: 'codex-final-decision.safe.json',
...
function runV119Gates(report, gateEnv) {
  ...
  const orchestration = buildOrchestrationReport({
    ...
    taskProfile: 'compatibility_repair',
    mutationTask: true,
    permissionEvidenceSource: 'external_owner_process_scope_not_machine_bound',
    safeNextAction: 'owner_merge_decision_only',
    finalDecisionClosureSafeNextAction: 'owner_merge_decision_only',
    workspaceIdentitySafeNextAction: 'owner_merge_decision_only',
    skillContextSafeNextAction: 'owner_merge_decision_only',
    ...
    forbiddenScopeProfile: 'TARGET_HARNESS_WORKFLOW_REPAIR_V127',
    ...
    nonGoals: ['product_code', 'runtime_operation', 'merge_or_approval_authority'],
  ...
  if (report.decisionCapsule) report.decisionCapsule = { ...report.decisionCapsule, harnessVersion: HARNESS_VERSION };
  report.orchestrationCapsule = orchestration.orchestrationCapsule;
  report.workerProofCapsule = workerProof.workerProofCapsule;
  report.ownerDecisionBrief = ownerBrief.ownerDecisionBrief;
  ...
  report.workerProofStatus = workerProof.workerProofStatus;
  report.reviewChainStatus = workerProof.reviewChainStatus;
  report.ownerDecisionBriefStatus = ownerBrief.ownerDecisionBriefStatus;
  report.finalDecisionPointerStatus = buildFinalDecisionPointerStatus(report);
  report.v119SelfTestStatus = selfTestStatus.status === 'fail' ? selfTestStatus : {
    ...selfTestStatus,
    status: selfTestStatus.status || 'pass',
  };
}

function initializeV119Statuses(report) {
  for (const key of V119_STATUS_KEYS) if (!report[key]) report[key] = { status: 'not_run' };
  if (!report.v119SelfTestStatus) report.v119SelfTestStatus = { status: 'not_run' };
}

function runV120Gates(report, gateEnv) {
  const selfTestStatus = process.env.CODEX_SKIP_V120_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : runGateScript('scripts/codex-v120-self-test.mjs', 'v120SelfTestStatus', 'CODEX_V120_SELF_TEST_REPORT', gateEnv);
  report.v120SelfTestStatus = selfTestStatus.status === 'fail' ? selfTestStatus : {
    ...selfTestStatus,
    status: selfTestStatus.status || 'pass',
  };
}

function initializeV120Statuses(report) {
...
function runV122Gates(report, gateEnv) {
...
function initializeV122Statuses(report) {
  if (!report.v122SelfTestStatus) report.v122SelfTestStatus = { status: 'not_run' };
}

function runV123Gates(report, gateEnv) {
  const selfTestStatus = process.env.CODEX_SKIP_V123_SELF_TEST === '1'
    ? { status: 'not_applicable', reasonCodes: ['self_test_recursion_guard'], safeSummaryOnly: true }
    : runGateScript('scripts/codex-v123-self-test.mjs', 'v123SelfTestStatus', 'CODEX_V123_SELF_TEST_REPORT', gateEnv);
  report.v123SelfTestStatus = selfTestStatus.status === 'fail' ? selfTestStatus : {
    ...selfTestStatus,
    status: selfTestStatus.status || 'pass',
  };
}

function initializeV123Statuses(report) {
  if (!report.v123SelfTestStatus) report.v123SelfTestStatus = { status: 'not_run' };
}

function runV124Gates(report, gateEnv) {
...
function initializeV124Statuses(report) {
  if (!report.v124SelfTestStatus) report.v124SelfTestStatus = { status: 'not_run' };
}

function runV125Gates(report, gateEnv) {
...
function initializeV125Statuses(report) {
  if (!report.v125SelfTestStatus) report.v125SelfTestStatus = { status: 'not_run' };
}

function runV126Gates(report, gateEnv) {
...
function initializeV126Statuses(report) {
...
function validateSourceHarness() {
  ...
  return {
    ...
    markerScan: {
      ...
      status: markerScanMissing.length ? 'fail' : 'pass',
      ...
      missingCount: markerScanMissing.length,



      mismatchCount: markerScanMismatches.length,
      historicalAbsorbed: markerScanMismatches.length > 0,
...
async function runSourceHarnessGate() {
  ...
  initializeV122Statuses(report);
  initializeV123Statuses(report);
  initializeV124Statuses(report);
...
async function main() {
  ...
  if (!process.env.CODEX_HARNESS_SOURCE_REPO && !process.env.CODEX_HARNESS_MODE && manifestForAutoMode?.sourceOnlyRelease === true) {
    ...
    process.env.CODEX_PROFILE_COMPAT_MODE = process.env.CODEX_PROFILE_COMPAT_MODE || 'optional';
  }
