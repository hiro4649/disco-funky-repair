...
export function buildTokenHardBudgetStatus(input = {}) {
  const operatorVisibleStatuses = Number(input.operatorVisibleStatuses ?? 0);
  const safeArtifactReads = Number(input.safeArtifactReads ?? 0);
  const reasonCodes = [];
  if (operatorVisibleStatuses > 12) reasonCodes.push('operator_visible_status_budget_exceeded');
  if (safeArtifactReads > 4) reasonCodes.push('safe_artifact_read_budget_exceeded');
  const metrics = { operatorVisibleStatuses, safeArtifactReads };
  return reasonCodes.length
    ? fail(reasonCodes, { metrics })
    : pass({ metrics });
}
...
export function validateHardSafetyClaims(input = {}) {
  ...
  return reasonCodes.length ? fail(reasonCodes) : pass({ hardSafetyClaimsPreserved: true });
}
