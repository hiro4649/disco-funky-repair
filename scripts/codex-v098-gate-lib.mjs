...
import { currentVersion, activeSelfTestSuite, activeSelfTestStatusKey } from './codex-harness-version.mjs';
...
export function buildRemoteProductSafeArtifacts(input = parseJson(process.env.CODEX_REMOTE_PRODUCT_EVIDENCE_RUNNER_JSON) || {}, env = process.env) {
  ...
  const artifactHarnessVersion = currentVersion || HARNESS_VERSION;
  const versionFields = {
    harnessVersion: artifactHarnessVersion,
    sourceHarnessVersion: artifactHarnessVersion,
    activeHarnessVersion: artifactHarnessVersion,
    targetHarnessVersion: artifactHarnessVersion,
    activeSelfTestSuite,
