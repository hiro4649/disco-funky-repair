...
const cases = [
  ...
  test('owner_decision_receipt_is_load_bearing_artifact', () => buildArtifactConsistencyReport({ head: 'abc' }).entries.some((entry) => entry.artifactName === 'codex-owner-decision-receipt.safe.json')),
  test('all_load_bearing_artifacts_are_checked', () => buildArtifactConsistencyReport({ head: 'abc' }).checkedArtifacts === 5),
