...
const Header = (props: {
  ...
  useEffect(() => {
    if (wallet.account?.address) {
      onSignUp(wallet.account?.address);
    }
  }, [wallet.account?.address, onSignUp]);
  ...
  return (
    <header
      className={`ease-out-linear fixed top-0 z-999 flex flex-col w-full duration-0 dark:border-stroke-dark dark:bg-[#00000066] ${props.isVisibleHeader == true ? "translate-y-0" : "-translate-y-full"}`}
    >
      <div className="flex flex-grow items-center justify-between px-3 py-2 md:px-5 2xl:px-10">
        <div className="flex items-center gap-x-3 sm:gap-4 relative">
          ...
          >
            {connectBonus
              ? <></>
              : <span className="absolute top-0 left-4 z-1 -mt-1 -mr-2 bg-red-500 text-white rounded-full text-xs w-4 h-4 flex items-center justify-center">
                1
              </span>
            }
