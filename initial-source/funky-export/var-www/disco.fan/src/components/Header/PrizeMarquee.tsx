...
const PrizeMarquee: React.FC = () => {
  ...
  const PrizeItem = ({ item, index }: { item: prizelist; index: number }) => (
    ...
    >
      <div className="w-6 h-6 rounded-full overflow-hidden mr-2 flex-shrink-0">
        <Image
          src={item.icon || `/images/new_nfts/${item.default_image}`}
          alt={item.token_name}
          width={24}
          height={24}
          ...
          onError={(e) => {
            ...
            target.src = `/images/new_nfts/${item.default_image}`;
          }}
        />
      </div>
      ...
      <span className="text-[#00FFCC] text-sm mr-3 flex-shrink-0">
  ...
  return (
    <div className="w-full overflow-hidden bg-gradient-to-r via-gray-900 to-black border-gray-700 relative">
      <div className="flex animate-css-marquee">
