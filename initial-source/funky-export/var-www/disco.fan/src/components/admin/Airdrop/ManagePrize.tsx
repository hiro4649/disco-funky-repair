    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (formAction === 'update') {
                ...
                res = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/admin/airdrop/prize/${selectedPrize?.id}`, data);
            } else {
                // Create Prize via POST request
                console.log(`Create::::${data.id}`)
                res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/admin/airdrop/prize/${selectedPrize?.id}`, data);
            }
    ...
    const renderCell = React.useCallback((prize: prize, columnKey: React.Key) => {
        ...
        switch (columnKey) {
            case "flag":
                return (
                    prize.flag
                        ? <Chip color="primary">Valid</Chip> : <Chip color="warning">Invalid</Chip>
                );
            case "symbol":
                return (
                    <User
                        ...
                        description={
                            <Link isExternal href={`https://suivision.xyz/coin/${prize.ca}`} size="sm">
                                {prize.token_name}
                            </Link>
                        }
                        name={prize.symbol}
            ...
            case "price":
                return (
                    <div className="text-[15px] text-[#00FFCC] flex justify-center items-center">
                        <span className="text-[11px] text-[#666666] pl-1">$  </span>
                        {formatNumber(prize.price ?? 0).count != 0 ? "0.0" : formatNumber(prize.price ?? 0).value}
                        <div className="text-[12px] -mb-2">{formatNumber(prize.price ?? 0).count == 0 ? "" : formatNumber(prize.price ?? 0).count}</div>
                        {formatNumber(prize.price ?? 0).count == 0 ? "" : formatNumber(prize.price ?? 0).value}
                    </div>
                );
            case "balance":
                return (
            ...
            case "action":
                return (
                    <Button color="primary" variant="shadow" onPress={() => { openPrizeSettingModal(prize) }}>
                        SETTING
                    </Button>
        ...
        <>
            ...
            >
                <ModalContent >
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">SETTING PRIZE</ModalHeader>
                            <ModalBody>
                                <Form
