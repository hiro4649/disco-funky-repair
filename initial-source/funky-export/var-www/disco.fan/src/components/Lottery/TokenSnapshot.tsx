...
const TokenSnapshot: React.FC<TokenSnapshotProps> = ({ onProgressComplete }) => {
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      if (tallyData.percentage === 100 && onProgressComplete) {
        onProgressComplete();
      }
    };
  ...
  useEffect(() => {
    ...
    const animateProgressBar = () => {
      // Animate skill bars
      const bars = document.querySelectorAll<HTMLElement>(".tally-bar-info");
      bars.forEach((bar) => {
      ...
      const percentElements =
        document.querySelectorAll<HTMLElement>(".tally-percent");
  ...
  return (
    <div>
      <div className="mb-[7px] flex items-center justify-between">
        <p className="text-[15px] text-white">{t('Next token snapshot')}</p>
        <div className="flex items-center gap-x-1 font-semibold">
          <div className="flex justify-center items-center mr-1">
            <p className="text-[15px] leading-[1.2rem] font-normal text-[#00FFCC] mr-[0.2rem]">
              {tallyTime.hours}
            </p>
          ...
          <p className="text-sm font-normal text-white">/ 6h</p>
        </div>
      </div>
      <div className="h-[16px] w-full rounded-[10px] bg-[#333333]">
        <div
          className="tally-bar-info flex h-[16px] items-center justify-end rounded-[10px] border-[1px] border-white pr-[6px]"
          data-total={tallyProgress}
        >
          <span className="tally-percent text-xs text-black">
