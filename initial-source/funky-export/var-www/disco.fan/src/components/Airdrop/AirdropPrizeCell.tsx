...
const AirdropPrizeCell: React.FC<any> = (data: {
  ...
  return (
    <div
      key={index}
      className={`airdrop-fade-in-left cursor-pointer rounded-[8px] border-y-[0.5px] border-y-[#666666] bg-white px-3.5 py-2.5 shadow-1 dark:bg-[#1D1B20] transition-all duration-500 ease-in-out ${data.addClass}`}
    ...
    >
      <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            <div className="flex gap-x-2">
              <div className="itmes-center flex flex-col">
                <div className="flex items-center justify-start text-[15px] font-semibold leading-[1.3rem] text-white text-center gap-x-[5px]">
                  <div>{item.symbol}</div>
                  <Image
                    width={11}
                    height={11}
                    src={`/images/logo/${item.default_image}`}
                    alt="Icon"
        ...
        <div className="flex gap-x-4">
          <span className="text-[1rem] text-[#FFFF33]" data-from="0" data-to={`${Number(item.quantity / item.price).toFixed(5)}`} ref={targetRef}>
            0
          </span>
        </div>
      </div>
